...
test.describe('People CRUD - Full Dataset', () => {
    ...
    if (isVisible) {
      ...
      const result = await page.evaluate(async () => {
        ...
        const r2 = await fetch(
          '/api/people/?page=1&pagesize=5&keys=gramps_id,primary_name',
          {
            headers: {Authorization: `Bearer ${token}`},
          }
        )
        ...
        return {
          ok: r2.ok && Array.isArray(data) && data.length > 0,
          type: 'list',
        }
      })
  ...
  test('should open person edit form', async ({page}) => {
    ...
    if (grampsId) {
      ...
      const editButton = page
        .locator(
          'mwc-icon-button[icon="edit"], button:has-text("Edit"), [aria-label="Edit"]'
        )
        .first()
  ...
  test('should create new person', async ({page}) => {
    ...
    const addButton = page
      .locator('mwc-fab[icon="add"], mwc-fab, button:has-text("Add")')
      .first()
