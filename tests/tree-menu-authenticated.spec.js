...
test.describe('Family Tree Menu (Authenticated)', () => {
  test.skip(
    ...
    'Skipping - no credentials provided'
  )
  ...
  test('should display Family Tree menu item after login', async ({page}) => {
    await page.screenshot({
      path: 'test-results/auth-01-after-login.png',
      fullPage: true,
    })
  ...
  test('should navigate to tree view when clicking Family Tree', async ({
    page,
  }) => {
    // Find and click Family Tree menu item
    const familyTreeLink = await page.locator(
      'grampsjs-list-item[href="/tree"]'
    )
    ...
    if ((await familyTreeLink.count()) > 0) {
      ...
      await page.screenshot({
        path: 'test-results/auth-02-tree-view.png',
        fullPage: true,
      })
      ...
      console.log(
        `🌳 Tree view component found: ${treeComponent > 0 ? 'Yes' : 'No'}`
      )
  ...
  test('should show tree visualization tabs', async ({page}) => {
    ...
    await page.screenshot({
      path: 'test-results/auth-03-tree-tabs.png',
      fullPage: true,
    })
    ...
    tabTexts.forEach(text => console.log(`  - ${text?.trim()}`))
