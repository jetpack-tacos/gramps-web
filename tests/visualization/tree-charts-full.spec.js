...
test.describe('Tree Visualizations - Phase 1 Regression', () => {
  ...
  test('should display tree view with tab navigation', async ({page}) => {
    ...
    const tabTexts = await Promise.all(
      Array.from({length: tabCount}, (_, index) =>
        tabs.nth(index).textContent()
      )
    )
    const trimmedTabTexts = tabTexts.map(text => text?.trim() || '')
    console.log(`   Tabs: ${tabTexts.join(', ')}`)

    expect(trimmedTabTexts.some(t => t.includes('Ancestor'))).toBe(true)
    expect(trimmedTabTexts.some(t => t.includes('Descendant'))).toBe(true)
    expect(trimmedTabTexts.some(t => t.includes('Hourglass'))).toBe(true)
    expect(trimmedTabTexts.some(t => t.includes('Fan'))).toBe(true)
    expect(trimmedTabTexts.some(t => t.includes('Relationship'))).toBe(true)
  ...
  test('should render Ancestor Tree chart', async ({page}) => {
    ...
    const count = await page
      .locator('svg g, svg rect, svg text, svg path, svg circle')
      .count()
  ...
  test('should render Descendant Tree chart', async ({page}) => {
    ...
    const count = await page
      .locator('svg g, svg rect, svg text, svg path')
      .count()
  ...
  test('should render Relationship Graph (force layout)', async ({page}) => {
    ...
    const count = await page
      .locator('svg circle, svg line, svg g, svg text')
      .count()
  ...
  test('should allow changing generations depth', async ({page}) => {
    ...
    console.log(
      `   Chart elements (confirms multi-generation rendering): ${nodeCount}`
    )
  ...
  test('should not show JavaScript errors', async ({page}) => {
    ...
    await Array.from({length: 5}).reduce(
      (promise, _, index) =>
        promise.then(async () => {
          await page.locator('md-tabs md-primary-tab').nth(index).click()
