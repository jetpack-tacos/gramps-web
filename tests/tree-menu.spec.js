...
test.describe('Family Tree Menu and Navigation', () => {
  ...
  test('should display the Family Tree menu item', async ({ page }) => {
    ...
    if (loginForm > 0) {
      ...
      if (await usernameField.isVisible() && await passwordField.isVisible()) {
        console.log('ℹ️  Login fields found. Add credentials to .env to automate login.');
      }
    ...
    const familyTreeLink = await page.locator('a[href="/tree"], grampsjs-list-item[href="/tree"]');
    ...
    if (linkCount === 0 && textCount === 0) {
      ...
      const allMenuItems = await page.locator('grampsjs-main-menu grampsjs-list-item').all();
      console.log(`  - Total menu items visible: ${allMenuItems.length}`);

      for (let i = 0; i < allMenuItems.length; i++) {
        const text = await allMenuItems[i].textContent();
        const href = await allMenuItems[i].getAttribute('href');
  ...
  test('should navigate to /tree directly', async ({ page }) => {
    ...
    await page.screenshot({ path: 'test-results/05-tree-direct-navigation.png', fullPage: true });
    ...
    if (currentUrl.includes('/tree')) {
      ...
      const treeChart = await page.locator('grampsjs-tree-chart, grampsjs-fan-chart, grampsjs-relationship-chart').count();
      ...
      if (treeView === 0 && treeChart === 0) {
        ...
        if (pageText.includes('No Home Person')) {
          console.log('ℹ️  Issue: No Home Person set - this is required for tree view');
        }
      }
    } else {
  ...
  test('should check which build is running', async ({ page }) => {
    ...
    // Check for version or build info in HTML
...
test.describe('Home Person Configuration', () => {
  ...
  test('should check for Home Person setting', async ({ page }) => {
    ...
    const homePersonComponent = await page.locator('grampsjs-home-person').count();
    console.log(`\n🏠 Home Person component found: ${homePersonComponent > 0 ? 'Yes' : 'No'}`);

    if (homePersonComponent > 0) {
      const homePersonText = await page.locator('grampsjs-home-person').textContent();
      console.log(`   Content: ${homePersonText?.trim().substring(0, 100)}`);

      if (homePersonText?.includes('Set') || homePersonText?.includes('No Home Person')) {
        console.log('   ⚠️  Home Person is NOT set - this may prevent tree view from working');
      } else {
        console.log('   ✅ Home Person appears to be configured');
      }
    }

    await page.screenshot({ path: 'test-results/06-home-person-check.png', fullPage: true });
  });
});
